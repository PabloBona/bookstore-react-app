...
function BookCard({
  books, handleRemoveBook, handleEditBook, handleCommentBook,
}) {
  return (
    <div className="container">
      {books.map((book) => (
        <div key={book.item_id} className="row bg-white border border-primary-subtle m-3 rounded">
          <div className="col-5">
            <div className="p-3">
              <p>Category</p>
              ...
              <div className="mod-books">
                <div className="remove link-separator mb-2">
                  <Button className="mb-2" onClick={() => handleCommentBook(book.item_id)}>
                    Comment
                  </Button>
                </div>
                <div className="remove link-separator mb-2">
                  <Button onClick={() => handleRemoveBook(book.item_id)}>Remove</Button>
                </div>
                <div className="remove mb-2">
                  <Button onClick={() => handleEditBook(book.item_id)}>Edit</Button>
                </div>
              </div>
            </div>
          </div>
...
BookCard.propTypes = {
  ...
  handleCommentBook: PropTypes.func.isRequired,
};
